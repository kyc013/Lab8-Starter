...
return new Promise((resolve, reject) => {
  ...
  RECIPE_URLS.forEach((recipeURL) => {
    // A5. TODO - Since we are going to be dealing with asynchronous code, create a try / catch block
    try {
      // A6. TODO - For each URL in that array, fetch the URL using the fetch API
      fetch(recipeURL)
        .then((response) => response.json())
        .then((recipe) => {
          // A7. TODO - Add the new recipe to the recipes array
          recipesArray.push(recipe);
          // A8. TODO - Check to see if you have finished retrieving all of the recipes
          if (recipesArray.length === RECIPE_URLS.length) {
            // A9. TODO - Save the recipes to storage using the function we have provided
            saveRecipesToStorage(recipesArray);
            // A10. TODO - Pass the recipes array to the Promise's resolve() method
            resolve(recipesArray);
          }
        })
        .catch((error) => {
          // A10-A11. TODO - Log any errors from catch using console.error and pass any errors to the Promise's reject() function
